...
nb_users = len(data_user_interset)
for i in range(nb_users):
    cohort_interests[int(data_cohort_id[i][1], 16)] = add_tabs(cohort_interests[int(data_cohort_id[i][1], 16)], data_user_interset[i])
    nb_users_by_cohort[int(data_cohort_id[i][1], 16)] += 1

fichier_output = open("processed_data/cohort_interests_" + str(year) + ".csv", "w",newline='')
...
for i in range(nb_cohorts) :
    if(nb_users_by_cohort[i] != 0) :
        ...
        writer.writerow([hex(i)] + cohort_interests[i])
